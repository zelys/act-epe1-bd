...
- Cantidad de Atenciones por medico especialidad.
  ...
  FROM Atenciones A, Medicos M, Especialidades E
  WHERE A.Med_Id = M.Med_Id AND M.Esp_Id = E.Esp_Id
  GROUP BY E.Esp_Id;
  ```
  ...
  SELECT E.Esp_Id, E.Esp_Glosa, COUNT(*) AS Cantidad_Atenciones
  FROM Atenciones A
  JOIN Medicos M ON A.Med_Id = M.Med_Id
  JOIN Especialidades E ON M.Esp_Id = E.Esp_Id
  GROUP BY E.Esp_Id;
  ```
...
- Total valor de prestaciones entre las fechas 01/01 2021 al 31/03/2021.
  ...
  SELECT C.Comu_Id, C.Comu_Glosa, SUM(P.Prest_Valor) AS Valor_Total
  FROM Atenciones A
  RIGHT JOIN Prestaciones P ON A.Prest_Id = P.Prest_Id
...
- Promedio de atenciones por profesional médico.
  ...
  FROM Medicos M, Atenciones A
  WHERE M.Med_Id = A.Med_Id
  GROUP BY A.Med_Id;
  ```
